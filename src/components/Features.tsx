    ...
    <section className="py-24 bg-primary-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-primary-900 mb-6">
            Why Choose{' '}
            <span className="text-primary-500">
              IEDC
            </span>
          </h2>
          <p className="text-lg sm:text-xl text-primary-900/70 max-w-3xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {features.map((feature, index) => {
            ...
            return (
              <div
                key={index}
                className="group relative bg-primary-50/80 backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-primary-500/20 hover:border-primary-500/50 transition-all duration-300 transform hover:scale-105 hover:-translate-y-2"
              >
                {/* Glow Effect */}
                ...
                <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-xl bg-gradient-to-r ${feature.gradient} flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300`}>
                  <IconComponent className="w-6 h-6 sm:w-8 sm:h-8 text-primary-50" />
                </div>

                {/* Content */}
                <h3 className="text-xl sm:text-2xl font-bold text-primary-900 mb-3 sm:mb-4">{feature.title}</h3>
                <p className="text-sm sm:text-base text-primary-900/70 leading-relaxed">{feature.description}</p>
